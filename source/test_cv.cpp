...
void cvIWait(nonstd::stop_token sToken, int id, bool &ready,
...
void testStdCV(bool callNotify) {
  ...
  {
    nonstd::jthread t1(
        [&ready, &readyMutex, &readyCV, callNotify](nonstd::stop_token it) {
          {
            std::unique_lock lg{readyMutex};
...
void testCVPred(bool callNotify) {
  ...
  {
    nonstd::jthread t1(
        [&ready, &readyMutex, &readyCV, callNotify](nonstd::stop_token st) {
          try {
            std::unique_lock lg{readyMutex};
            readyCV.wait(lg, st, [&ready] { return ready; });
...
void testCVStdThreadNoPred(bool callNotify) {
  ...
  {
    std::thread t1(
        [&ready, &readyMutex, &readyCV, st = is.get_token(), callNotify] {
          {
            std::unique_lock lg{readyMutex};
            bool ret = readyCV.wait(lg, st, [&ready] { return ready; });
            if (ret) {
              std::cout << "t1: ready" << std::endl;
  ...
  nonstd::stop_source is;
  {
    std::thread t1(
        [&ready, &readyMutex, &readyCV, st = is.get_token(), callNotify] {
          ...
          {
            std::unique_lock lg{readyMutex};
            ret = readyCV.wait(lg, st, [&ready] { return ready; });
            if (ret) {
              std::cout << "t1: ready" << std::endl;
              assert(!st.stop_requested());
            } else {
...
void testMinimalWait(int sec) {
  ...
  try {
    ...
    {
      nonstd::jthread t1(
          [&ready, &readyMutex, &readyCV, dur](nonstd::stop_token st) {
            try {
              std::cout << "\n- start t1" << std::endl;
      ...
      std::cout << "- leave scope (should signal interrupt and unblock CV wait)"
                << std::endl;
    } // leave scope of t1 without join() or detach() (signals cancellation)
    std::cout << "\n*** OK" << std::endl;
  } catch (const std::exception &e) {
...
void testMinimalWaitFor(int sec1, int sec2) {
  ...
  try {
    ...
    std::condition_variable_any2 readyCV;
    {
      nonstd::jthread t1([&ready, &readyMutex, &readyCV, durInt,
                          durWait](nonstd::stop_token st) {
        try {
          std::cout << "\n- start t1" << std::endl;
          auto t0 = std::chrono::steady_clock::now();
          {
            std::unique_lock lg{readyMutex};
            readyCV.wait_for(lg, st, durWait, [&ready] { return ready; });
          }
...
void testTimedCV(bool callNotify, bool callInterrupt, Dur dur) {
  ...
  std::cout << "*** start testTimedCV(callNotify=" << callNotify << ", "
            ...
            << std::chrono::duration<double, std::ratio<1>>(dur).count() << "s)"
            << std::endl;
  using namespace std::literals;
  ...
  {
    nonstd::jthread t1([&ready, &readyMutex, &readyCV, callNotify,
                        dur](nonstd::stop_token st) {
      std::cout << "\n- start t1" << std::endl;
      auto t0 = std::chrono::steady_clock::now();
      int timesDone{0};
      while (timesDone < 3) {
        {
          std::unique_lock lg{readyMutex};
          auto ret = readyCV.wait_for(lg, st, dur, [&ready] { return ready; });
          if (dur > 5s) {
...
void testTimedIWait(bool callNotify, bool callInterrupt, Dur dur) {
  ...
  {
    nonstd::jthread t1([&ready, &readyMutex, &readyCV, callNotify, dur,
                        &t1Feedback](nonstd::stop_token st) {
      std::cout << "\n- start t1" << std::endl;
      auto t0 = std::chrono::steady_clock::now();
      int timesDone{0};
      while (timesDone < 3) {
...
template <int numExtraCV> void testManyCV(bool callNotify, bool callInterrupt) {
...

  {
    ...
    std::array<bool, numExtraCV>
        arrReady{}; // don't forget to initialize with {} here !!!
    std::array<std::mutex, numExtraCV> arrReadyMutex{};
    std::array<std::condition_variable_any2, numExtraCV> arrReadyCV{};
    std::vector<nonstd::jthread> vThreadsDeferred;

    nonstd::jthread t0(cvIWait, 0, std::ref(ready), std::ref(readyMutex),
                       std::ref(readyCV), callNotify);
    {
      ...
      std::vector<nonstd::jthread> vThreads;
      for (int idx = 0; idx < numExtraCV; ++idx) {
        std::this_thread::sleep_for(0.1ms);
        nonstd::jthread t([idx, t0stoken = t0ssource.get_token(), &arrReady,
                           &arrReadyMutex, &arrReadyCV, callNotify] {
          // use interrupt token of t0 instead
          // NOTE: disables signaling interrupts directly to the thread
          cvIWait(t0stoken, idx + 1, arrReady[idx], arrReadyMutex[idx],
                  arrReadyCV[idx], callNotify);
        });
...
int main() {
  try {
    ...
    std::cout << "\n\n**************************\n";
    testManyCV<9>(false, false); // don't call notify, don't call request_stop()
                                 // (implicit interrupt)
  } catch (const std::exception &e) {
