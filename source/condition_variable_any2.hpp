...
public:
  ...
  template <class Lockable, class Predicate>
  bool wait(Lockable &lock, nonstd::stop_token stoken, Predicate pred);

  // return:
  // - true if pred() yields true
  // - false otherwise (i.e. on timeout or interrupt)
  template <class Lockable, class Clock, class Duration, class Predicate>
  bool wait_until(Lockable &lock, nonstd::stop_token stoken,
                  const chrono::time_point<Clock, Duration> &abs_time,
                  Predicate pred);
  // return:
  // - true if pred() yields true
  // - false otherwise (i.e. on timeout or interrupt)
  template <class Lockable, class Rep, class Period, class Predicate>
  bool wait_for(Lockable &lock, nonstd::stop_token stoken,
...
inline bool condition_variable_any2::wait_until(
    Lockable &lock, nonstd::stop_token stoken,
...
template <class Lockable, class Rep, class Period, class Predicate>
inline bool
condition_variable_any2::wait_for(Lockable &lock, nonstd::stop_token stoken,
                                  const chrono::duration<Rep, Period> &rel_time,
                                  Predicate pred) {
