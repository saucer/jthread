...
void testCVDeadlock() {
  ...
  {
    nonstd::jthread t1([&ready, &readyMutex, &readyCV](nonstd::stop_token st) {
      std::cout << "\n"
                << std::this_thread::get_id() << ": t1: lock " << &readyMutex
                << std::endl;
    ...
    nonstd::jthread t2([&ready, &readyMutex, &readyCV, &t1StopSource](
                           nonstd::stop_token /*t2_stop_token_not_used*/) {
      std::cout << "\n"
                << std::this_thread::get_id() << ": t2: lock " << &readyMutex
                << std::endl;
      std::unique_lock<std::mutex> lg{readyMutex};
      std::cout << "\n"
