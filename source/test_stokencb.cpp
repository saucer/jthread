...
TEST(RequestingStopOnSourceUpdatesToken) {
  nonstd::stop_source s;
  nonstd::stop_token t = s.get_token();
  CHECK(t.stop_possible());
  CHECK(!t.stop_requested());
  s.request_stop();
  CHECK(t.stop_requested());
...
TEST(CallbackNotExecutedImmediatelyIfStopNotYetRequested) {
  ...
  {
    nonstd::stop_callback cb(s.get_token(), [&] { callbackExecuted = true; });
  }
...
TEST(CallbackExecutedImmediatelyIfStopAlreadyRequested) {
  ...
  bool executed = false;
  nonstd::stop_callback r{s.get_token(), [&] { executed = true; }};
  CHECK(executed);
}
...
TEST(RegisterMultipleCallbacks) {
  ...
  nonstd::stop_callback r1{t, callback};
  nonstd::stop_callback r2{t, callback};
  nonstd::stop_callback r3{t, callback};
  nonstd::stop_callback r4{t, callback};
  nonstd::stop_callback r5{t, callback};
  nonstd::stop_callback r6{t, callback};
  nonstd::stop_callback r7{t, callback};
  nonstd::stop_callback r8{t, callback};
  nonstd::stop_callback r9{t, callback};
  nonstd::stop_callback r10{t, callback};
...
TEST(ConcurrentCallbackRegistration) {
  auto threadLoop = [](nonstd::stop_token token) {
    std::atomic<bool> cancelled = false;
    while (!cancelled) {
      nonstd::stop_callback registration{token, [&] { cancelled = true; }};

      nonstd::stop_callback cb0{token, [] {}};
      nonstd::stop_callback cb1{token, [] {}};
      nonstd::stop_callback cb2{token, [] {}};
      nonstd::stop_callback cb3{token, [] {}};
      nonstd::stop_callback cb4{token, [] {}};
      nonstd::stop_callback cb5{token, [] {}};
      nonstd::stop_callback cb6{token, [] {}};
      nonstd::stop_callback cb7{token, [] {}};
      nonstd::stop_callback cb8{token, [] {}};
      nonstd::stop_callback cb9{token, [] {}};
      nonstd::stop_callback cb10{token, [] {}};
      nonstd::stop_callback cb11{token, [] {}};
      nonstd::stop_callback cb12{token, [] {}};
      nonstd::stop_callback cb13{token, [] {}};
      nonstd::stop_callback cb14{token, [] {}};
      nonstd::stop_callback cb15{token, [] {}};
      nonstd::stop_callback cb16{token, [] {}};
...
TEST(CallbackDeregisteredFromWithinCallbackDoesNotDeadlock) {
  nonstd::stop_source src;
  std::optional<nonstd::stop_callback<std::function<void()>>> cb;
...
TEST(CallbackDeregistrationDoesNotWaitForOtherCallbacksToFinishExecuting) {
  ...
  std::optional<nonstd::stop_callback<decltype(dummyCallback) &>> cb1{
  ...
  nonstd::stop_callback blockingCb{src.get_token(), [&] {
  ...
  std::optional<nonstd::stop_callback<decltype(dummyCallback) &>> cb2{
      std::in_place, src.get_token(), dummyCallback};

  std::thread signallingThread{[&] { src.request_stop(); }};

  // Wait until the callback starts executing on the signalling-thread.
  // The signalling thread will remain blocked in this callback until we release
  // it.
  {
    std::unique_lock lock{mut};
  ...
  // Finally, signal the callback to unblock and wait for the signalling thread
  // to finish.
  {
    std::unique_lock lock{mut};
...
TEST(CallbackDeregistrationBlocksUntilCallbackFinishes) {
  ...
  std::thread callbackRegisteringThread{[&] {
    ...
    {
      nonstd::stop_callback cb{src.get_token(), [&] {
                                 std::unique_lock lock{mut};
                                 callbackExecuting = true;
                                 cv.notify_all();
                                 lock.unlock();
...
template <typename CB> struct callback_batch {
  using callback_t = nonstd::stop_callback<CB &>;

  callback_batch(nonstd::stop_token t, CB &callback)
...
TEST(CancellationSingleThreadPerformance) {
  ...
  for (int i = 0; i < iterationCount; ++i) {
    nonstd::stop_callback r{s.get_token(), callback};
  }
  ...
  auto report = [](const char *label, auto time, std::uint64_t count) {
    ...
    std::cout << label << " took " << ms << "ms ("
              << (ns / static_cast<double>(count)) << " ns/item)" << std::endl;
  };
