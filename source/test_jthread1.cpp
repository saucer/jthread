...
void testJThreadWithout() {
  ...
  assert(nonstd::jthread::hardware_concurrency() ==
         std::thread::hardware_concurrency());
  nonstd::stop_token stoken;
  assert(!stoken.stop_possible());
  {
    nonstd::jthread::id t1ID{std::this_thread::get_id()};
    std::atomic<bool> t1AllSet{false};
    nonstd::jthread t1([&t1ID, &t1AllSet] { // NOTE: no stop_token passed
...
void testThreadWithToken() {
  ...
  nonstd::stop_source ssource;
  nonstd::stop_source origsource;
  ...
  assert(!ssource.stop_requested());
  {
    ...
    nonstd::jthread t1(
        [&t1ID, &t1AllSet, &t1done](nonstd::stop_token st) {
          // check some values of the started thread:
          t1ID = std::this_thread::get_id();
          t1AllSet.store(true);
...
void testJoin() {
  ...
  assert(ssource.stop_possible());
  {
    nonstd::jthread t1([](nonstd::stop_token stoken) {
      // wait until interrupt is signaled (due to calling request_stop() for the
      // token):
    ...
    nonstd::jthread t2([ssource]() mutable {
      for (int i = 0; i < 10; ++i) {
...
void testDetach() {
  ...
  std::atomic<bool> t1FinallyInterrupted{false};
  {
    ...
    nonstd::stop_token t1InterruptToken;
    std::atomic<bool> t1AllSet{false};
    nonstd::jthread t1([&t1ID, &t1IsInterrupted, &t1InterruptToken, &t1AllSet,
                        &t1FinallyInterrupted](nonstd::stop_token stoken) {
      // check some values of the started thread:
      t1ID = std::this_thread::get_id();
    ...
    // and check all values:
    assert(!t0.joinable());
    // assert(nonstd::stop_source{} == t0.get_stop_source());
    assert(t1.joinable());
    assert(t1ID == t1.get_id());
...
void testTemporarilyDisableToken() {
  ...
  nonstd::stop_source t1is;
  {
    nonstd::jthread t1([&state](nonstd::stop_token stoken) {
      ...
      nonstd::stop_token interruptDisabled;
...
void testJThreadAPI() {
  ...
  nonstd::jthread t0;
  nonstd::jthread::native_handle_type nh = t0.native_handle();
  assert((std::is_same_v<decltype(nh), std::thread::native_handle_type>));
  assert(!t0.joinable());
  nonstd::stop_source ssourceStolen{std::move(ssource)};
  assert(!ssource.stop_possible());
...

  {
    ...
    std::atomic<bool> t1AllSet{false};
    nonstd::jthread t1(
        [&t1ID, &t1InterruptToken, &t1AllSet](nonstd::stop_token stoken) {
          // check some values of the started thread:
          t1ID = std::this_thread::get_id();
          t1InterruptToken = stoken;
          assert(stoken.stop_possible());
    ...
    std::swap(t0, t1);
    assert(!t1.joinable());
    assert(nonstd::stop_token{} == t1.get_stop_source().get_token());
    assert(nonstd::stop_token{} == t1.get_stop_token());
    assert(t0.joinable());
    ...
    t1 = std::move(ttmp);
    assert(!t0.joinable());
    assert(nonstd::stop_token{} == t0.get_stop_source().get_token());
    assert(nonstd::stop_token{} == t0.get_stop_token());
    assert(t1.joinable());
    assert(t1ID == t1.get_id());
    assert(t1InterruptToken == t1.get_stop_source().get_token());
    assert(t1InterruptToken == t1.get_stop_token());
