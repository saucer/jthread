...
void interruptByDestructor() {
  ...
  bool t1WasInterrupted = false;
  {
    ...
    nonstd::jthread t1([interval,
                        &t1WasInterrupted](nonstd::stop_token stoken) {
...
void interruptStartedThread() {
...

  {
    ...
    nonstd::jthread t1([interval, &interrupted](nonstd::stop_token stoken) {
      printID("t1 STARTED with interval " + asString(interval) + " with id");
      try {
        // loop until interrupted (at most 40 times the interval)
        for (int i = 0; i < 40; ++i) {
...
void interruptStartedThreadWithSubthread() {
  ...
  auto interval = 0.2s;
  {
    ...
    nonstd::jthread t1([interval](nonstd::stop_token stoken) {
      printID("t1 STARTED with id");
      nonstd::jthread t2([interval, stoken] {
...
void basicAPIWithFunc() {
  ...
  {
    std::cout << "\n- start jthread t1" << std::endl;
    nonstd::jthread t(&foo, "foo() called in thread with id: ");
...
void testExchangeToken() {
  ...
  {
    std::cout << "\n- start jthread t1" << std::endl;
    std::atomic<nonstd::stop_token *> itPtr = nullptr;
    nonstd::jthread t1([&itPtr](nonstd::stop_token sToken) {
    ...
    nonstd::stop_token it;
    itPtr.store(&it);

    std::this_thread::sleep_for(interval);
    std::cout << "\n- replace by valid/stoppable token" << std::endl;
    auto isTmp = nonstd::stop_source{};
    it = nonstd::stop_token{isTmp.get_token()};
    itPtr.store(&it);
...
void testConcurrentInterrupt() {
  ...
  nonstd::stop_source is;
  {
    ...
    std::cout << "\n- start jthread t1" << std::endl;
    nonstd::jthread t1([it = is.get_token()](nonstd::stop_token stoken) {
      printID("t1 STARTED (id: ", ") printing . or - or i");
      try {
        char c = ' ';
    ...
    std::vector<nonstd::jthread> tv;
    int requestStopNumTrue = 0;
    for (int i = 0; i < numThreads; ++i) {
      std::this_thread::sleep_for(0.1ms);
      nonstd::jthread t([&t1, &requestStopNumTrue] {
...
void testJthreadMove() {
  std::cout << "\n*** start testJthreadMove()" << std::endl;
  {
    ...
    nonstd::jthread t1{[&interruptSignaled](nonstd::stop_token st) {
...
void testEnabledIfForCopyConstructor_CompileTimeOnly() {
  ...
  {
    nonstd::jthread t1;
    // nonstd::jthread t2{t1};  // should not compile
  }
  std::cout << "\n*** OK" << std::endl;
}
