...
void testStopCallbackInits() {
  ...
  nonstd::stop_callback cb10{token, stop10};
  static_assert(
      std::is_same_v<decltype(cb10)::callback_type, decltype(stop10)>);

  std::cout << "-----\n";
  auto stop11 = [] { std::cout << "stop11\n"; };
  nonstd::stop_callback cb11{token, std::move(stop11)};
  static_assert(
  ...
  nonstd::stop_callback cb12{token, std::ref(stop12)};
  ...
  std::cout << "-----\n";
  nonstd::stop_callback cb13{token, [] { std::cout << "stop13\n"; }};
  static_assert(!std::is_reference_v<decltype(cb13)::callback_type>);

  std::cout << "----- function<void()>:\n";
  nonstd::stop_callback<std::function<void()>> cb14{
  ...
  nonstd::stop_callback cb15{token, stop15};
  static_assert(
  ...
  nonstd::stop_callback<std::function<void()>> cb16{token, stop16};
  static_assert(
  ...
  auto cb17 = [token] {
    ...
    return nonstd::stop_callback{token, f};
  }();
  static_assert(
  ...
  ImplicitArg i;
  nonstd::stop_callback<MyCallback> cb18{token, i};
  ...
  ExplicitArg e;
  nonstd::stop_callback<MyCallback> cb19{token, e};
  ...
  // ERROR:
  // nonstd::stop_callback<MyCallback> cb =
